...
The model is an orchestrator.  Intelligence is carried by the atom/state
structures and their continuous dynamics; this module does not introduce a
Transformer block or token-token attention path.
"""
...
class ToroidalFractalIntelligence(nn.Module):
    def __init__(self, vocab_size=32000, d_model=256, n_modes=256, n_atoms_max=1024):
        super().__init__()
        self.encoder = ToroidalEncoder(vocab_size=vocab_size, d_model=d_model, n_atoms_max=n_atoms_max)
        self.state = FractalSuperpositionState(n_modes=n_modes, d_model=d_model)
        self.dynamics = RK4DynamicsEngine(ToroidalDynamics(d_model=d_model, n_modes=n_modes), dt=0.1, n_steps=4)
        self.interaction = ToroidalInteraction(d_model=d_model, n_modes=n_modes)
    ...
    def forward(self, token_ids: torch.Tensor, context_ids: torch.Tensor | None = None) -> dict:
        if token_ids.dim() > 1:
        ...
        alpha_new = self.dynamics.evolve(alpha, input_token=new_atom.r if new_atom is not None else None, context=None)
        ...
        if len(self.atoms) > 10:
            aggregates, _ = self.aggregation.aggregate(
                self.atoms.r, self.atoms.phi, self.atoms.omega, self.atoms.E,
                self.atoms.kappa, self.atoms.M, self.atoms.tau, self.atoms.rho,
            )
        ...
        for agg in aggregates:
            ...
            if abs_id >= 0:
                abstractions.append({"id": abs_id, "pattern": self.abstraction.abstraction_memory[abs_id].detach()})
                self.abstraction_count += 1

        persistent_state = None
        if len(self.energy_history) > 5:
            E = self.atoms.E.mean(dim=0) if len(self.atoms) else alpha_new.mean(dim=0)
            ...
            persistent_state, consolidate_mask = self.consolidation.consolidate(alpha_new, E.unsqueeze(0), stability.unsqueeze(0))
            if consolidate_mask.any():
                self.consolidation_count += int(consolidate_mask.sum().item())
        else:
            if len(self.atoms):
        ...
        logits = logits.mean(dim=0, keepdim=True).expand(n_tokens, -1)

        # Commit the continuous state only after constructing the differentiable
        # output.  This prevents .data assignment from severing the learning path.
        with torch.no_grad():
    ...
    def train_step(self, token_ids, target_ids, optimizer, context_ids=None):
        ...
        E = self.atoms.E.mean() if len(self.atoms) else alpha.new_zeros(())
        ...
        return {
            "loss": loss.item(), "sparsity_loss": sparsity_loss.item(), "energy_loss": energy_loss.item(),
            "total_loss": total_loss.item(), "n_atoms": len(self.atoms),
            "n_aggregates": len(output["aggregates"]), "n_abstractions": len(output["abstractions"]),
    ...
    def generate(self, prompt_ids, max_length=100, temperature=1.0, top_k=50):
        ...
        for _ in range(max_length):
            output = self.forward(generated[-1:].unsqueeze(0))
            token_id, _, _ = self.production.sample(self.state.get_field(), output["persistent_state"], output["abstractions"], temperature=temperature, top_k=top_k)
    ...
    def save(self, path):
        torch.save({
            "encoder": self.encoder.state_dict(), "state": self.state.get_state_dict(),
            "dynamics": self.dynamics.state_dict(), "interaction": self.interaction.state_dict(),
            "aggregation": self.aggregation.state_dict(), "abstraction": self.abstraction.state_dict(),
            "consolidation": self.consolidation.state_dict(), "production": self.production.state_dict(),
            "atoms": self.atoms.state_dict(), "energy_history": self.energy_history,
            "training_loss": self.training_loss, "consolidation_count": self.consolidation_count,
            "abstraction_count": self.abstraction_count,
            "config": {"vocab_size": self.encoder.vocab_size, "d_model": self.encoder.d_model, "n_modes": self.state.n_modes, "n_atoms_max": self.encoder.n_atoms_max},
        }, path)
    ...
    def get_shared_params_summary(self):
        return {
            ...
            "n_atoms": len(self.atoms), "n_modes": self.state.n_modes, "d_model": self.encoder.d_model,
        }

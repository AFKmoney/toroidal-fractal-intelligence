"""Final scaling study with proper metrics."""
import sys
sys.path.insert(0, '.')
import torch
import time
...
print("="*70)
print(f"\n{'Config':<12} {'Params':<12} {'Atoms':<8} {'InitLoss':<8} {'FinalLoss':<8} {'Improvement':<12} {'Atoms/Param':<12}")
print("-"*80)
for r in results:
    print(f"{r['config']['name']:<12} {r['parameters']:<12,} {r['final_atoms']:<8} {r['initial_loss']:<8.4f} {r['final_loss']:<8.4f} {r['loss_improvement']:<12.4f} {r['atoms_per_parameter']:<12.6f}")

# Save results
with open("./results/scaling_study_final.json", "w") as f:
    json.dump(results, f, indent=2)

print(f"\nResults saved to: ./results/scaling_study_final.json")
print("="*70)

...
print("Testing LARGE config...")

model = ToroidalFractalIntelligence(vocab_size=100, d_model=256, n_modes=256, n_atoms_max=512)
n_params = sum(p.numel() for p in model.parameters())
print(f"Params: {n_params:,}")

tokens = list(range(100)) * 50
sequences = [torch.tensor(tokens[i:i+256]) for i in range(0, len(tokens)-256, 128)]
loader = InfiniteDataLoader(sequences, batch_size=4, seq_len=256)
